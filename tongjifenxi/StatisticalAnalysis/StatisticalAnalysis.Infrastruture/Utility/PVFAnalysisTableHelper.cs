...
{
    public static class PVFAnalysisTableHelper
    {
        /// <summary>
        /// 数据源表的类型
        /// </summary>
        ...
        /// 将峰谷平表转换为以时间为字段的横表
        /// </summary>
        /// <param name="source">原表，应有以下字段,[]为可选：（timeStamp,PeakB,ValleyB,FlatB）</param>
        ...
        private static DataTable VerticalToHorizontalYearly(DataTable source, int year)
        {
            ...
            DataRow drSharp = destination.NewRow();//尖峰期
            ...
            for (int i = 0; i < source.Rows.Count; i++)
            {
                ...
                drFlat[columnName] = (decimal)source.Rows[i]["FlatB"];
                drSharp[columnName] = (decimal)source.Rows[i]["MorePeakB"];
            }

            destination.Rows.Add(drPeak);
            destination.Rows.Add(drValley);
            destination.Rows.Add(drFlat);
            destination.Rows.Add(drSharp);
        ...
        private static DataTable VerticalToHorizontalMonthly(DataTable source, int year, int month)
        {
            ...
            DataRow drValley = destination.NewRow();
            DataRow drFlat = destination.NewRow();
            DataRow drSharp = destination.NewRow();
