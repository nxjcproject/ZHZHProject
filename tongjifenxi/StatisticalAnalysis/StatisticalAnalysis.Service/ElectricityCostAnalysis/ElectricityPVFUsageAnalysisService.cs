...
namespace StatisticalAnalysis.Service.ElectricityCostAnalysis
{
    public static class ElectricityPVFUsageAnalysisService
    {
        ...
        /// <returns></returns>
        public static DataTable GetElectricityPVFUsageCustom(string organizationId, DateTime startTime, DateTime endTime, bool price = true)
        {
            DataTable pvfUsage = GetElectricityPVFDailyUsageByOrganiztionId(organizationId, startTime, endTime);
        ...
        /// <returns></returns>
        public static DataTable GetElectricityPVFUsageMonthly(string organizationId, int year, int month, bool price = true)
        {
        ...
        /// <returns></returns>
        public static DataTable GetElectricityPVFUsageYearly(string organizationId, int year, bool price = true)
        {
            DateTime startTime = DateTime.Parse(year + "-01-01 00:00:00");
            DateTime endTime = DateTime.Parse(year + "-12-31 23:59:59");

            DataTable pvfUsage = GetElectricityPVFMonthlyUsageByOrganiztionId(organizationId, startTime, endTime);
        ...
        internal static DataTable GetElectricityPVFDailyUsageByOrganiztionId(string organizationId, DateTime startTime, DateTime endTime)
        {
        ...
        /// <param name="endTime">结束时间</param>
        /// <returns>峰谷平用电量</returns>
        internal static DataTable GetElectricityPVFMonthlyUsageByOrganiztionId(string organizationId, DateTime startTime, DateTime endTime)
        {
            string connectionString = ConnectionStringFactory.NXJCConnectionString;
            ISqlServerDataFactory dataFactory = new SqlServerDataFactory(connectionString);
        ...
        private static DataTable GetPriceTable(string organizationId, DataTable originalTable)
        {
            ...
            double sharpPrice = double.Parse(price.Rows[0]["SharpElectrovalence"].ToString());
            foreach (DataRow dr in originalTable.Rows)
            {
                ...
                if (originalTable.Columns.Contains("MorePeakB"))
                    dr["MorePeakB"] = decimal.Parse((double.Parse(dr["MorePeakB"].ToString()) * peakPrice).ToString("0.00"));
            }
